...
## Eliminación de la tendencia con el método de diferenciación
...
Luego de estimar la tendencia (sin importar el método elegido) se debe eliminar de la serie, posteriormente se estima la estacionalidad y también se elimina. A los valores resultantes se les llama residuos. 

```{r estima-estacionalidad}

d <- 12          # periodo estacional 
q <- d / 2
...
# Serie SIN ESTACIONALIDAD x_des = x_t - s_hat_t 
serie_df$sin_estacionalidad <- serie_df$ingresos - serie_df$s_hat
...
plot_ly(serie_df, x = ~fecha) %>%
  add_lines(y = ~residuo, name = "Residuo",
            line = list(color = "firebrick", width = 2)) %>%
  layout(
    title = "Residuos de la serie (descomposición aditiva)",
    xaxis = list(title = "Fecha"),
    yaxis = list(title = "Ingresos"),
    legend = list(orientation = "h", y = -0.2)
  )
