...
El proceso de estimación de la tendencia  $\hat{s}_t$ requiere que primero se estime la tendencia (por el método que sea), posteriormente:

- Agrupar por mes: se juntan los valores correspondientes al mismo mes de distintos años, suponiendo que la estacionalidad se repite anualmente.
- Obtener el efecto estacional: se promedian los valores de cada mes para estimar cuánto se desvía típicamente ese mes del nivel general.
- Centrar los índices: se ajustan los 12 promedios para que su suma sea cero, de modo que representen únicamente el efecto estacional.
- Repetir el patrón: los 12 índices estacionales se asignan a cada año según el mes correspondiente.
- Desestacionalizar: se resta el patrón estacional estimado a la serie original, eliminando las fluctuaciones periódicas pero conservando la tendencia.

En resumen, para estimar la estacionalidad se elimina temporalmente la tendencia para identificar el patrón que se repite cada año, luego se estima un índice para cada mes y luego ese patrón se resta de la serie original para obtener una serie sin estacionalidad. 
...
La implementación de forma manual en R:
...
Existen paqueterías que realizan el proceso de estimación para cada componente de forma automática, la forma más popular es con la función `decompose()` de R base, otra alternativa es `ts_decompose()` de `TSstudio`
