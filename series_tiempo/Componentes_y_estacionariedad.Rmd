...
A continuación el proceso de forma manual.

```{r filtro-mm-manual, echo=TRUE}

media_movil_manual <- function(x, k, tipo = c("centrada", "trailing")) {
  tipo <- match.arg(tipo)
  n <- length(x)
  ma <- rep(NA_real_, n)
  ...
  if (tipo == "trailing") {
    for (i in k:n) {
      suma <- 0
      for (j in (i - k + 1):i) suma <- suma + x[j]   # suma manual de la ventana
      ma[i] <- suma / k
    }
  } else {
    if (k %% 2 == 0) stop("Para 'centrada' usa un k impar (simplifica la ventana simétrica)")
    half <- k %/% 2
    for (i in (half + 1):(n - half)) {
      suma <- 0
      for (j in (i - half):(i + half)) suma <- suma + x[j]  # suma manual de la ventana
      ma[i] <- suma / k
    }
  }
  ma
...
k    <- 12              # ventana impar -> centrada, fácil de visualizar
half <- k %/% 2
 
serie_df$ma_manual <- media_movil_manual(serie_df$ingresos, k = k, tipo = "trailing")
 
# rango de "centros" válidos para la ventana centrada
centros_validos <- (half + 1):(n - half)
 

plot(serie_df$ma_manual) 
# ---------------------------------------------------------------
# 3. Construcción de los "frames" para la animación
#    (patrón de acumulación: en el frame f se revela todo lo
#     calculado hasta ese punto, más la ventana actual resaltada)
# ---------------------------------------------------------------
...
En este caso es de utilidad complementar la comparativa con métricas de evaluación como el RMSE y el MAE para determinar cuál de los métodos es más adecuado para estimar la tendencia de la serie.
